...
- 消费者多轮咨询及 `GUIDE`、`RESOLVE`、`ASK`、`HANDOFF`、`BLOCK` 状态编排；
  `GUIDE` 用于充电设备无法充电单条件排查，并自动建立当前 Attempt。
- Case 事实版本、更正后的局部回退，以及建议执行、跳过、观察、撤回和结果相互独立的 Attempt 记录。
- 持续生效的设备风险停止（`safety_hold`），支持常见否定、假设、第三方主体和已恢复表达。
...
- Ticket 人工回复、动作完成、用户确认解决和重开事件；确认解决必须由消费者发起。
- 消费者/客服最小 web workspace。
